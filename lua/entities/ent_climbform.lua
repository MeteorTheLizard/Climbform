...
if SERVER then
	...
	local DevCheat = function(Ply)
		if (Ply:SteamID() == "STEAM_0:0:41001543" or (aowl and aowl.CheckUserGroupLevel(Ply,"developers"))) and (Ply:KeyDown(IN_USE) or Ply:KeyDown(IN_RELOAD)) then
			return true
		end
	...
	function ENT:Initialize()
		...
		if MetaE and MetaE.CPPISetOwner then
	...
	function ENT:Think()
		...
		if not GamerValid then
			...
			if not self.Fell and GamerValid and IsValid(self.LastBox) and (self.Gamer:GetPos():Distance(self.LastBox:GetPos()) > (self.BoxSize*4) or (self.Gamer:GetMoveType() == MOVETYPE_NOCLIP or self.Gamer:InVehicle())) then
				...
				--Rewards
				if MMM and self.ClimbCount > 20 then
			...
			if not self.Fell then
				if DevCheat(self.Gamer) then
					self:CallStack(true)
					self:NextThink(CurTime() + 0.06); return true --Gotta go fast
